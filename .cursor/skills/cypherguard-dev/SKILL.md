...
| P0 | M18 对话输入栏重构 | 🆕 待开发 | 模型选择/网络搜索/清空消息/引用知识库 移入输入框工具栏 |
...
## M18: 对话输入栏重构（Chat Toolbar）

**问题**: 当前模型选择、问答模式等控件放在侧边栏，占用空间且操作不直觉。应将核心对话控件下沉到输入框工具栏中，类似现代 AI 聊天界面。

**目标**: 在对话输入框下方添加工具栏，包含 4 个图标按钮：

| 图标 | 功能 | 交互 |
|------|------|------|
| Cpu | 选择模型 | 弹出下拉：显示可用 chat 模型列表 + 问答模式(quick/strict) + Agent 开关 |
| Globe | 网络搜索 | 切换按钮：开启/关闭 Web 搜索补充 |
| Trash2 | 清空消息 | 确认后清空当前对话消息 |
| BookOpen | 引用知识库 | 弹出下拉：显示知识库列表，可选择引用范围 |

**涉及文件**:
- `frontend/src/pages/Chat.tsx` — ChatInputArea 重构 + Toolbar 组件
- `frontend/src/pages/Dashboard.tsx` — 移除侧边栏的模式/模型选择器
- `frontend/src/services/api.ts` — 添加清空对话消息 API (如需)

**实现步骤**:
1. Chat.tsx: ChatInputArea 新增工具栏，4 个图标按钮各自含弹出层
2. Dashboard.tsx: 侧边栏仅保留"新建对话" + 对话列表，模式/模型/Agent 全移入 Chat Toolbar
3. Chat props 扩展: 传入 chatModels、selectedModelId、模式控制等
4. 工具栏样式: 图标 + 文字标签，激活态高亮，弹出下拉用 AnimatePresence
5. 保持移动端响应式

**验收**: 侧边栏干净只有对话列表，所有对话控制集中在输入栏工具栏。
...
### 后端改动自动重启容器
修改后端代码后，**必须**重启对应的 Docker 容器使改动生效。文件与容器的映射关系：

| 改动文件路径 | 需重启的容器（docker compose restart） |
|-------------|---------------------------------------|
| `services/chat-service/**` | `chat-service` |
| `services/kb-service/**` | `kb-service` |
| `services/model-gateway/**` | `model-gateway` |
| `services/auth-service/**` | `auth-service` |
| `services/worker/**` | `worker` |
| `shared/python/common/**` | `chat-service kb-service model-gateway auth-service worker`（共享代码，全部重启） |
| `gateway/nginx.conf` | `gateway` |
| `frontend/src/**` | `frontend`（volume 挂载 + Vite HMR，通常自动生效；若不生效则 restart） |
| `infra/db/migrate.sql` | 无需重启，执行迁移脚本 `infra/db/run_migrate.ps1` |

**执行命令**（在项目根目录）：
```powershell
docker compose restart <service-name> [<service-name2> ...]
```

示例：改了 `shared/python/common/schemas.py`，执行：
```powershell
docker compose restart chat-service kb-service model-gateway auth-service worker
```

**注意**：此操作在每次后端代码改动后自动执行，无需用户确认。
